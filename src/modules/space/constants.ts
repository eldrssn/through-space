export const PLANET_COUNT = 100
export const MIN_SCALE = 1 // Уменьшенное минимальное значение масштаба
export const DEFAULT_SCALE = 6
export const MAX_SCALE = 10.0 // Увеличенное максимальное значение масштаба
export const ZOOM_SENSITIVITY = 1 // Шаг приближения
export const WIDTH = 1920 * 3
export const HEIGHT = 1080 * 3

// Увеличенный фактор параллакса для более глубокой карты
export const PARALLAX_DEPTH_FACTOR = 0.005

// Диапазон значений scale для отображения всплывающей подсказки
export const MAX_GOBAL_SCALE = MAX_SCALE - ZOOM_SENSITIVITY
